package ch.fhnw.pfcs;//  --------  Interface to OpenGL 3.0  --------------
//                                        E. Gutknecht Juli 2015
import javax.media.opengl.*;
import javax.media.opengl.awt.*;
import ch.fhnw.util.math.*;


public interface MyRenderer1
{

    public GLCanvas getGLCanvas();                         // OpenGL-Window

    //  --------  Vertex-Methoden  -----------

    public void setColor(float r, float g, float b);       // aktuelle Vertexfarbe setzen

    void putVertex(float x, float y, float z);             // Vertex-Daten in Buffer speichern
    void putVertex(double x, double y, double z);

    public void copyBuffer(GL gl,int nVertices);           // Vertex-Array in OpenGL-Buffer kopieren

    public void rewindBuffer(GL gl);                       // Bufferposition zuruecksetzen


    //  --------  ModelView-Transformation  -----------

    //  ------------  Kamera-System  --------------------

    public void setCameraSystem(GL4 gl, float r,                                     // Abstand der Kamera von O
                                float elevation,                                     // Elevationswinkel in Grad
                                float azimut);                                       // Azimutwinkel in Grad


    public void setCameraSystem(GL4 gl, Vec3 A, Vec3 B, Vec3 up);                    // LookAt-Positionierung


    //  ---------  Operationen fuer ModelView-Matrix  --------------------

    public void rotate(GL4 gl, float phi, float x, float y, float z);                 // Objekt-System drehen, phi in Grad

    public void translate(GL4 gl, float x, float y, float z);                         // Objekt-System verschieben

    public void scale(GL4 gl, float scale);                                           // Skalierung Objekt-System

    public void rotateCam(GL4 gl, float phi, float x, float y, float z);              // Kamera-System drehen, phi in Grad

    public void translateCam(GL4 gl, float x, float y, float z);                      // Kamera-System verschieben

    public void pushMatrix(GL4 gl);                                                   // ModelView-Matrix speichern

    public void popMatrix(GL4 gl);                                                    // ModelView-Matrix vom Stack holen

    public Mat4 getModelViewMatrix(GL4 gl);

    public void setModelViewMatrix(GL4 gl, Mat4 M);

    public void loadIdentity(GL4 gl);                                                 // Rueckstellung auf Einheitsmatrix


    //  ---------  Projektion auf Bildebene -------------------

    public void setOrthogonalProjection(GL4 gl, float left, float right,           // Grenzen des ViewingVolumes
                                      float bottom, float top,
                                      float near, float far);


    public void setPerspectiveProjection(GL4 gl, float left, float right,          // Grenzen des ViewingVolumes
                                      float bottom, float top,
                                      float near, float far);


}
